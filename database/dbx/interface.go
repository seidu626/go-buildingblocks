...
// DBX limited interface with high-level API for pgx methods safe to be used in high-level business logic packages.
...
type DBX interface {
	...
	// affects the beginning command. i.e. there is no auto-rollback on context cancellation.
	BeginTx(ctx context.Context, txOptions pgx.TxOptions) (pgx.Tx, error)

	Querier
}

// Querier interface with methods used for everything, including transactions.
...
// Validate if the DBX interface was derived from *pgx.Conn and *pgxpool.Pool correctly.
var (
	_ DBX = (*pgx.Conn)(nil)
	_ DBX = (*pgxpool.Pool)(nil)
)
